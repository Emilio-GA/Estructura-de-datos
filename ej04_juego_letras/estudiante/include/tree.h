/**
 *  @file   tree.h
 *  @brief  Archivo de especificación del tree.h
 ...
 *
 * @tparam T is the type of the labels in each node. This type has to implement the following operations:
 *
...
public:
   ...
   *
   * @details This class represents a node of the tree. A node is determined by its label and the nodes it is connected
   * to (its parent, its leftmost child, and its righthand sibling).
   *
   * @author Emilio Guillen
   *
   * @author Alberto Rodriguez
...
private:
  ...
  /**
   * @brief Internal representation of the tree.
   */
  node root;
   ...
   *
   * @return True If Subtrees Hanging From both nodes are equal, false otherwise.
   */
   ...
   *
   * This method copies the tree structure hanging from a source node into a destiny node.
   *
   * @param src Nodo tipo source.
   *
   * @param dest Nodo tipo destino.
   */
   ...
   *
   * @param prefix Prefix to print previous to the node label. It is used to properly format the depth of the tree.
   *
...
public:
   ...
   *
   * Given a node n and a tree child, inserts child as left child of n. If a previous subtree exists as a child, it
   ...
   *
   * Given a node n and a label, inserts a child with the label as left child of n. If a previous subtree exists as a
   * child, it becomes the righthand sibling of the new child.
   *
   * @param n Node that inherits the subtree as a child.
   *
   ...
   *
   * Given a node n and a tree sibling, inserts sibling as right sibling of n. If a previous subtree exists as a sibling,
   * it becomes the righthand sibling of the inserted one.
   *
   ...
   *
   * Given a node n and a label, inserts a right sibling with the label for n. If a previous subtree exists as a sibling,
   * it becomes the righthand sibling of the new one.
   *
   ...
   *
   * It copies the content of other tree into current tree, destroying the previous content if needed.
   *
   * @param other Tree to be copied.
   *
   ...
   *
   * Removes the leftmost child of the specified node n. If there are remaining children (as left siblings of the pruned
   * node), first sibling is set as current left children. The pruned tree is returned using dest.
   *
   ...
   *
   * Removes the righthand sibling of the specified node n. If there are remaining siblings (as left siblings of the
   * pruned node), first sibling is set as current right sibling. The pruned tree is returned using dest.
   *
   * @param n Node to be pruned.
   *
   * @param dest Tree to return the pruned subtree. It is modified.
   ...
   *
   * Given a complete tree, it empties its content and frees the memory.
   */
  void clear();
   ...
   *
   * Checks if a certain node is a leaf of the tree. A node is considered a leaf if it has no children.
   *
   * @param v node to be checked.
   *
   ...
   *
   * Equality operator overload. Two trees are considered to be equal if they have the same structure, and the labels of
   * the corresponding nodes in each tree are the same.
   *
   ...
   *
   * Inequality operator overload. Two trees are considered to be equal if they have the same structure, and the labels
   * of the corresponding nodes in each tree are the same.
   *
   * @param other tree to be compared with current.
   *
  ...
  private:
    ...
    /**
     * @brief Struct nodewrapper.
     *
     * @details Node content encapsulation. A node is fully determined by its label, a pointer to its parent node, a
     * pointer to its leftmost child, and a pointer to its righthand sibling. This structure lies in dynamic memory.
     */
    struct nodewrapper{
  ...
  public:
     ...
     *
     * Checks if the current node is null, i.e., if the internal nodewrapper does not have allocated memory.
     *
